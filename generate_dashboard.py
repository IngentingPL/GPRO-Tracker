...
function getCalendarNextRace() {{
    ...
    // ROZBUDOWA: filtruj po dacie - wybierz TYLKO przyszłe wyścigi
    // Pobierz daty z kalendarza i przefiltruj
    ...
    // Jeśli znaleziono przyszły wyścig, zwróć go
    if (nextFutureRace) return nextFutureRace;

    // Fallback: jeśli brak dat lub wszystkie w przeszłości, użyj starej logiki
...
function resolvePredictionContext(pred) {{
    ...
    const sameTrack =
        normalizeTrackName(predictedRace.track) ===
        normalizeTrackName(calendarNextRace.track);
    ...
    if (predictedRace.track || predictedSeason !== null || predictedRaceNo !== null) {{
        staleReason =
            `Prediction.json: ${{predictedRace.track || 'Nieznany tor'}} ` +
            `(S${{predictedSeason ?? '?'}} R${{predictedRaceNo ?? '?'}}) · ` +
            `Kalendarz: ${{calendarNextRace.track || 'Nieznany tor'}} ` +
            `(S${{calendarSeason ?? '?'}} R${{calendarRaceNo ?? '?'}})`;
    }}
